	...
	class Main
	{
        ...
        void change_pk_skill_desc()
        {
            ...
            if (id_sel >= 0) 
            {
                ...
                pk::get_skill(특기_불굴).desc = pk::u8encode(pk::format("병력 \x1b[23x{}\x1b[0x 미만시 공격을 {}% 확률로 회피", val1, val2) );
            }
            ...
            if (id_sel >= 0) 
            {
                ...
                pk::get_skill(특기_금강).desc = pk::u8encode(pk::format("받은 피해가 \x1b[23x{}\x1b[0x 미만시 {}% 확률로 회피", val1, val2) );
            }
            ...
            if (id_sel >= 0) 
            {
                ...
                pk::get_skill(특기_노발).desc = pk::u8encode(pk::format("전법으로 공격받았을 때 기력을 \x1b[23x+{}\x1b[0x", val1) );
            }
            ...
            if (id_sel >= 0)
            {
                ...
                pk::get_skill(특기_내조).desc = pk::u8encode(pk::format("부부의 능력치가 \x1b[23x+{}\x1b[0x 증가", val1) );
            }
			...
			pk::get_skill(특기_답파).desc = pk::u8encode("벼랑길 이동시 피해 없음. 낙석과 불함정 피해 경감");
			// pk::get_skill(특기_운반).desc = pk::u8encode("");
			pk::get_skill(특기_해독).desc = pk::u8encode("독천의 피해 없음");	
			...
			pk::get_skill(특기_매복).desc = pk::u8encode("복병 성공시 크리티컬");
			...
			pk::get_skill(특기_정묘).desc = pk::u8encode("적부대 격파시 기교 2배");
			...
			pk::get_skill(특기_백마).desc = pk::u8encode("기사연구없이 기사 가능. 기사시 크리티컬");
			pk::get_skill(특기_보좌).desc = pk::u8encode("주장일 때 아군부대 공격시 \x1b[23x30%\x1b[0x 확률로 지원. 혐오무장 제외");
			// pk::get_skill(특기_불굴).desc = pk::u8encode("");
			// pk::get_skill(특기_금강).desc = pk::u8encode("");)
			pk::get_skill(특기_철벽).desc = pk::u8encode("적의 일제공격을 파훼");
			// pk::get_skill(특기_노발).desc = pk::u8encode("");
			pk::get_skill(특기_등갑).desc = pk::u8encode("피해 \x1b[23x-50%\x1b[0x. 불로 인한 피해 \x1b[22x+100%\x1b[0x");
			pk::get_skill(특기_강운).desc = pk::u8encode("전사나 포박, 부상을 당하지 않음");
			...
			pk::get_skill(특기_신장).desc = pk::u8encode("무력 우위시 통상 공격 및 적중한 전법 크리티컬");
			...
			pk::get_skill(특기_호위).desc = pk::u8encode("동료 무장이 전사나 부상당하지 않음");
			pk::get_skill(특기_화공).desc = pk::u8encode("지력 우위시 화계 항상 성공");
			pk::get_skill(특기_언독).desc = pk::u8encode("지력 우위시 위보 항상 성공");
			pk::get_skill(특기_기략).desc = pk::u8encode("지력 우위시 교란 항상 성공");
			pk::get_skill(특기_궤계).desc = pk::u8encode("지력 우위시 동토 항상 성공");
			pk::get_skill(특기_허실).desc = pk::u8encode("지력 우위시 계략 항상 성공");
			pk::get_skill(특기_묘계).desc = pk::u8encode("지력 우위시 적중한 계략 크리티컬");
			pk::get_skill(특기_비책).desc = pk::u8encode("지력 열세시 적중한 계략 크리티컬");
			pk::get_skill(특기_간파).desc = pk::u8encode("지력 우위시 적의 계략 간파"); 
			pk::get_skill(특기_통찰).desc = pk::u8encode("적의 모든 계략 간파");
			pk::get_skill(특기_화신).desc = pk::u8encode("지력 우위시 화계 성공, 피해 증가. 불에 우대");
			pk::get_skill(특기_신산).desc = pk::u8encode("지력 우위시 모든 계략이 성공하고 크리티컬. 계략에 우대");
			pk::get_skill(특기_백출).desc = pk::u8encode("모든 계략의 소모기력이 \x1b[23x1\x1b[0x이 됨");
			...
			pk::get_skill(특기_규율).desc = pk::u8encode("계략 위보를 간파");
			pk::get_skill(특기_침착).desc = pk::u8encode("계략 교란을 간파");
			pk::get_skill(특기_명경).desc = pk::u8encode("계략 위보, 교란을 간파");
			// pk::get_skill(특기_주악).desc = pk::u8encode("");
			pk::get_skill(특기_시상).desc = pk::u8encode("군악대 회복 기력 \x1b[23x+100%\x1b[0x");
			pk::get_skill(특기_축성).desc = pk::u8encode("군사 시설 내구도 상승량 \x1b[23x+100%\x1b[0x");
			pk::get_skill(특기_둔전).desc = pk::u8encode("소속 관문, 항구의 병량소모 없음");
			pk::get_skill(특기_명성).desc = pk::u8encode("징병으로 모집하는 병사가 증가");
			...
			pk::get_skill(특기_친오).desc = pk::u8encode("소속영내 오환 근거지 미출현");
			pk::get_skill(특기_친강).desc = pk::u8encode("소속영내 강 근거지 미출현");
			pk::get_skill(특기_친월).desc = pk::u8encode("소속영내 산월 근거지 미출현");
			pk::get_skill(특기_친만).desc = pk::u8encode("소속영내 남만 근거지 미출현");
			pk::get_skill(특기_위압).desc = pk::u8encode("소속영내 도적, 이민족 침략감소");
