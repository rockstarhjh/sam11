...
namespace 야전부대기동최적화
{
    ...
    const bool 간접병기_회피기동 = true;        // true=적용, false=미적용 (노병,정란,투석: 회피기동)
    const bool 충차부대_회피기동 = true;        // true=적용, false=미적용 (충차: 회피기동)
	...
	class Main
	{
		...
		void onTurnStart(pk::force@ force)
		{
            ...
            if (!pk::is_campaign())
			{
                ...
                if (pk::is_normal_force(force_id) and pk::get_scenario().difficulty == 난이도_특급 and !force.is_player())
                {
                    // 관문항구 공성전
                    if (관문항구_공성전기동)
                    {
        ...
        void func_archer_evasion(pk::unit@ unit)
        {
            ...
            if (unit.weapon >= 병기_주가 or unit.weapon == 병기_군마) return;   // 수군, 기병 제외
        ...
        bool check_valid_evasion_unit(pk::unit@ unit)
        {
            ...
            if (unit.status != 부대상태_통상) return false;  // 정상부대가 아니면 종료
        ...
        void run_order_combat_unit(pk::point prev_pos, pk::unit@ unit, pk::unit@ enemy, int range)
        {
            ...
            {
                return ((a.attr.stat[부대능력_공격] + a.attr.stat[부대능력_방어]) > (b.attr.stat[부대능력_공격] + b.attr.stat[부대능력_방어]));
            });
        ...
        pk::point set_atk_target(pk::unit@ unit, pk::point evasion_pos, int max_range)
        {
            ...
            {
                pk::set_order(unit, 부대임무_공격, target.pos); 
                return target.pos;
            }
            return -1;
        }
        ...
        pk::building@ get_siege_target_base(pk::unit@ unit)
        {
            ...
            for (int i = 0; i < int(range.length); i++)
			{
                ...
                for (int j = 0; j < int(arr_t.length); j++)
                {
                    ...
                    if (!hex.has_building and !hex.has_unit and pk::is_alive(target_base) 
                        and pk::is_enemy(unit, target_base) and target_base.get_id() < 건물_거점끝
            ...
            {
                bool city_a = (a.get_id() < 건물_도시끝);
                bool city_b = (b.get_id() < 건물_도시끝);
        ...
        bool unit_movement_control(pk::unit@ unit, pk::point evasion_pos, bool is_func262=false)
        {
            ...
            else if (move_distance > 1) // 회피기동 이동거리 > 1
            {
                ...
                {
                    pk::set_order(unit, 부대임무_이동, evasion_pos);   // 회피기동 목적지로 이동명령
                    pk::run_order(unit);
            ...
            if (is_moved)
            {
                ...
                if (!unit.action_done and unit.weapon != 병기_충차 and unit.order != 부대임무_공격)
                {
                ...
                if (디버그모드)
                {
                    ...
                    pk::info(pk::format("야전기동: {}대({}) 기동", unit_name, get_weapon_name(unit.weapon)));
                }
            }
        ...
        void func_siege_prior_atk(pk::building@ dst_base, pk::force@ force)
        {
            ...
            for (int i = 0; i < int(arr_siege_unit.length); i++)
            {
                ...
                {
                    pk::set_order(siege_unit, 부대임무_공격, dst_base.pos);
                    ...
                    if (디버그모드) 
                    {
                        ...
                        pk::info(pk::format("공성기동: {}대({}) 우선 {} 공격", siege_unit_name, weapon_name, base_name));
                    }
                }
            }
        }
        ...
        {
            if (!pk::is_alive(dst_base) or dst_base.get_id() >= 건물_거점끝) return;
            ...
            for (int i = 0; i < int(arr_range.length); i++)
            {
                ...
                if (pk::is_valid_pos(pos_t) and (pk::get_building_id(pos_t) == dst_base.get_id() or pk::get_distance(pos_t, dst_base.pos) <= 4))
                {
                    ...
                    if (pk::is_alive(unit) and unit.get_force_id() == force.get_id() and !pk::is_player_controlled(unit) and !pk::is_in_water(unit) and !unit.action_done and unit.status == 부대상태_통상)
                    {
        ...
        bool PushSiegeRamFormation(pk::building@ dst_base, pk::force@ force)
        {
            ...
            string base_name = pk::decode(pk::get_name(dst_base));
            if (디버그모드) pk::info(pk::format("공성기동: {}→{}, 근접공성부대수={}, 인접일반부대수={}", force_name, base_name, list_siegeram_unit.count, list_combat_unit_n1.count));
            ...
            if (list_combat_unit_n1.count > 0 and num_empty_pos == 0)
            {
                ...
                if (디버그모드) pk::info(pk::format("공성기동: {}대 후퇴좌표({},{})", combat_unit_name, evasion_pos.x, evasion_pos.y));
            ...
            if (!siege_unit.action_done and num_empty_pos > 0)
            {
                ...
                if (디버그모드) 
                {
                    ...
                    pk::info(pk::format("공성기동: {}대({}) {} 근접공성", siege_unit_name, weapon_name, base_name));
                }
                return true;
            }
            return false;
        ...
        bool PushSiegeTwrFormation(pk::building@ dst_base, pk::force@ force)
        {
            ...
            string force_name = pk::decode(pk::get_name(kunshu));
            string base_name = pk::decode(pk::get_name(dst_base));
            if (디버그모드) pk::info(pk::format("공성기동: {}→{}, 간접공성부대수={}, 일반부대수={}", force_name, base_name, list_range_unit.count, list_combat_unit_n2.count));
            ...
            if (!siege_unit.action_done and num_empty_pos > 0)
            {
                ...
                if (디버그모드) 
                {
                    ...
                    pk::info(pk::format("공성기동: {}대({}) {} 간접공성", siege_unit_name, weapon_name, base_name));
                }
                return true;
            }
            return false;
        ...
        {
            if (!pk::is_alive(dst_base) or !pk::is_normal_force(force) or dst_base.get_id() >= 건물_거점끝) return;
        ...
        void sort_list_combat_unit_n1()
        {
            ...
            {
                bool skill_a = (a.has_skill(특기_공신) or a.has_skill(특기_공성));
                bool skill_b = (b.has_skill(특기_공신) or b.has_skill(특기_공성));
                ...
                if (wpn_a == 병기_노 and wpn_b == 병기_노)
                {
                    ...
                    return (a.attr.stat[부대능력_공격] > b.attr.stat[부대능력_공격]);
                }
        ...
        void sort_list_combat_unit_n2()
        {
            ...
            list_combat_unit_n2.sort(function(a, b)
            {
                int wpn_a = pk::get_ground_weapon_id(a);
                int wpn_b = pk::get_ground_weapon_id(b);
                if (wpn_a != 병기_노 and wpn_b != 병기_노) return (a.attr.stat[부대능력_공격] < b.attr.stat[부대능력_공격]);
                if (wpn_a == 병기_노 and wpn_b != 병기_노) return false;
                if (wpn_a != 병기_노 and wpn_b == 병기_노) return true;
        ...
        int count_empty_neighbor_pos(pk::building@ dst_base, pk::unit@ unit, int distance=1)
        {
            ...
            if (디버그모드) pk::info(pk::format("공성기동: 빈좌표수={}", int(arr_empty_pos.length)));
