...
namespace 병기반송
{
	...
	//  일반 병기 중 한 품목이라도 40000 이상인 , 적 침공에서 안전한 병력 2천 이상 관문, 항구에서 보급 보내라
			...
			if ( (40000 <= pk::get_weapon_amount(base, 병기_창) or 40000 <= pk::get_weapon_amount(base, 병기_극) or 40000 <= pk::get_weapon_amount(base, 병기_노) or 40000 <= pk::get_weapon_amount(base, 병기_군마)) and 2000 <= pk::get_troops(base) and 건물_관문시작 <= base.get_id() and base.get_id() < 건물_항구끝 and no_enemy_around(base) )
				return true;


	// 일반 병기 중 한 품목이라도 80000 이상인, 적 침공에서 병력 4천 이상 안전한 도시에서 보급 보내라

			if ( (80000 <= pk::get_weapon_amount(base, 병기_창) or 80000 <= pk::get_weapon_amount(base, 병기_극) or 80000 <= pk::get_weapon_amount(base, 병기_노) or 80000 <= pk::get_weapon_amount(base, 병기_군마)) and 4000 <= pk::get_troops(base) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and no_enemy_around(base)  )
				return true;
	...
	if (대사표시_설정)             
            pk::say(pk::u8encode("일반병기 재고 초과다! \n다른 곳으로 보급한다!"), leader);
	...
	// 병기들이 각각 6만 이하로, 적 침공에서 안전한 도시로 보급 보내라
                    
if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_도시시작 <= dst_id and dst_id < 건물_도시끝 and (60000 <= pk::get_weapon_amount(dst, 병기_창) and 60000 <= pk::get_weapon_amount(dst, 병기_극) and 60000 <= pk::get_weapon_amount(dst, 병기_노) and 60000 <= pk::get_weapon_amount(dst, 병기_군마)) and no_enemy_around(dst) and func_already (dst) <= 6  )
                    {
                        best_dst = dst_id;
           ...
           //병기들이 각각 25000 이하로, 적 침공에서 안전한 관문, 항구로 보급 보내라

else if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_관문시작 <= dst_id and dst_id < 건물_항구끝 and (25000 <= pk::get_weapon_amount(dst, 병기_창) and 25000 <= pk::get_weapon_amount(dst, 병기_극) and 25000 <= pk::get_weapon_amount(dst, 병기_노) and 25000 <= pk::get_weapon_amount(dst, 병기_군마)) and no_enemy_around(dst) and base2_troops <= pk::get_max_troops(dst) * 0.7 and func_already (dst) <= 6 and 건물_파양항 != dst_id and 건물_노릉항 != dst_id and 건물_서하항 != dst_id and 건물_하양항 != dst_id and 건물_해현항 != dst_id and 건물_강도항 != dst_id)
                    {
                        best_dst = dst_id;
                        dst_list.add(dst);  // 가능 거점리스트 추가
                    }
	...
	//  공성 병기 중 한 품목이라도 60 이상인 , 적 침공에서 안전한 병력 2천 이상 관문, 항구에서 보급 보내라

			if ( (60 <= pk::get_weapon_amount(base, 병기_충차) or 60 <= pk::get_weapon_amount(base, 병기_정란) or 60 <= pk::get_weapon_amount(base, 병기_투석) or 60 <= pk::get_weapon_amount(base, 병기_목수)) and 2000 <= pk::get_troops(base) and 건물_관문시작 <= base.get_id() and base.get_id() < 건물_항구끝 and no_enemy_around(base)  )
				return true;


	// 공성 병기 중 한 품목이라도 60 이상인, 적 침공에서 안전한 병력 4천 이상 도시에서 보급 보내라

			if ( (60 <= pk::get_weapon_amount(base, 병기_충차) or 60 <= pk::get_weapon_amount(base, 병기_정란) or 60 <= pk::get_weapon_amount(base, 병기_투석) or 60 <= pk::get_weapon_amount(base, 병기_목수)) and 4000 <= pk::get_troops(base) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and no_enemy_around(base) )
				return true;
	...
	if (대사표시_설정)             
            pk::say(pk::u8encode("공성병기 재고 초과다! \n다른 곳으로 이동시켜라!"), leader);
...
if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_도시시작 <= dst_id and dst_id < 건물_도시끝 and  (40 <= pk::get_weapon_amount(dst, 병기_충차) or 40 <= pk::get_weapon_amount(dst, 병기_정란) or 40 <= pk::get_weapon_amount(dst, 병기_투석) or 40 <= pk::get_weapon_amount(dst, 병기_목수)) and no_enemy_around(dst) and func_already (dst) <= 6  )
                    {
                        best_dst = dst_id;
                        dst_list.add(dst);  // 가능 거점리스트 추가
                    }
...
else if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_관문시작 <= dst_id and dst_id < 건물_항구끝 and  (60 <= pk::get_weapon_amount(dst, 병기_충차) or 60 <= pk::get_weapon_amount(dst, 병기_정란) or 60 <= pk::get_weapon_amount(dst, 병기_투석) or 60 <= pk::get_weapon_amount(dst, 병기_목수)) and no_enemy_around(dst) and base2_troops <= pk::get_max_troops(dst) * 0.7 and func_already (dst) <= 6 and 건물_파양항 != dst_id and 건물_노릉항 != dst_id and 건물_서하항 != dst_id and 건물_하양항 != dst_id and 건물_해현항 != dst_id and 건물_강도항 != dst_id)
                    {
                        best_dst = dst_id;
                        dst_list.add(dst);  // 가능 거점리스트 추가
                    }
	...
	//  수상 병기 중 한 품목이라도 70 이상인 , 적 침공에서 안전한 벙력 2천 이상 관문, 항구에서 보급 보내라
			...
			if ( (70 <= pk::get_weapon_amount(base, 병기_누선) or 70 <= pk::get_weapon_amount(base, 병기_투함) ) and 2000 <= pk::get_troops(base) and 건물_관문시작 <= base.get_id() and base.get_id() < 건물_항구끝 and no_enemy_around(base)  )
				return true;


	// 수상 병기 중 한 품목이라도 70 이상인, 적 침공에서 안전한 병력 4천 이상 도시에서 보급 보내라

			if ( (70 <= pk::get_weapon_amount(base, 병기_누선) or 70 <= pk::get_weapon_amount(base, 병기_투함) ) and 4000 <= pk::get_troops(base) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and no_enemy_around(base)  )
				return true;
...
if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_관문시작 <= dst_id and dst_id < 건물_항구끝 and  (40 <= pk::get_weapon_amount(dst, 병기_누선) or 40 <= pk::get_weapon_amount(dst, 병기_투함) )  and no_enemy_around(dst) and base2_troops <= pk::get_max_troops(dst) * 0.7 and func_already (dst) <= 6 and 건물_파양항 != dst_id and 건물_노릉항 != dst_id and 건물_서하항 != dst_id and 건물_하양항 != dst_id and 건물_해현항 != dst_id and 건물_강도항 != dst_id)
                    {
                        best_dst = dst_id;
                        dst_list.add(dst);  // 가능 거점리스트 추가
                    }
...
else if (src_id != dst_id and src.get_force_id() == dst.get_force_id() and pk::is_alive(src) and pk::is_alive(dst) and 건물_도시시작 <= dst_id and dst_id < 건물_도시끝 and  (40 <= pk::get_weapon_amount(dst, 병기_누선) or 40 <= pk::get_weapon_amount(dst, 병기_투함) ) and no_enemy_around(dst) and func_already (dst) <= 6  )
                    {
                        best_dst = dst_id;
                        dst_list.add(dst);  // 가능 거점리스트 추가
                    }
