...
namespace 야전부대교전최적화
{
    ...
    const int  업데이트_주기설정 = 1;       // 주의 : 업데이트 주기를 2 또는 3으로 설정 시 유닛이 많아지면 게임이 느려질 수 있음
    ...
    const bool 아군거점_수성지원 = false;   // 근접 전투 부대 주변의 교전 중인 거점에 병력 적을 시 거점 진입하여 수성 지원
    ...
    const bool 적거점_점령성격적용 = false;   // 점령 허용 시 부대 주장의 성격에 따른 행동 (true=대담,저돌만 점령실행  / false=성격 관계없이 점령실행)
    ...
    const float 도시_병량부족_부대_초과한도기준 = 0.7f;   // 병량이 부족한 부대가 도시 진입시 한도초과 기준설정 (0.0f~1.0f) , 2024.03.06 일송정 수정
    const float 항관_병량부족_부대_초과한도기준 = 0.4f;   // 병량이 부족한 부대가 항관 진입시 한도초과 기준설정 (0.0f~1.0f) , 2024.03.06 일송정 수정
    const float 병량부족_거점_초과한도기준 = 0.5f;   // 병량이 부족하여 부대지원이 필요한 거점의 한도초과 기준설정 (0.0f~1.0f) , 2024.03.06 일송정 수정
    ...
    const bool 퇴각루트_적부대회피 = false;    // 군량, 병력 부족으로 아군 거점으로의 퇴각시 경로에 적부대가 있으면 원소속지로 향함. 
    ...
    const bool 혐오로_인한_전방위험감지_실패 = true;   // 전방국경_침입_후퇴에서, 전방 거점 내 장수가 태수, 도독, 군사, 군주 중 한사람만 혐오해도 후퇴 작동 안하게 하는 장치	
	...
	class Main
	{
        ...
        bool collapse_base_nearby(pk::unit@ unit)
        {
            ...
            if (pk::is_alive(target_base) and 건물_관문시작 <= target_base.get_id() and target_base.get_id() < 건물_항구끝)
            {
              ...
              /*  //   and enemy_approach_direct(target_base) 사용했었음. enemy_around_distance(target_base, 6) 대신 */				
        ...
        bool retreat_battlefield_nearby(pk::unit@ unit) 
        {
            ...
            if ((weapon_type == 0 or weapon_type == 1 or weapon_type == 4) and int(unit.troops) >= 극소병력부대_복귀병력기준 and unit.food >= float (부대복귀_병량기준_비율 * unit.troops )) return false;		
            if ((weapon_type == 2 or weapon_type == 3) and int(unit.troops) >= 극소병력부대_복귀병력기준 and unit.food >= float (부대복귀_병량기준_비율 * unit.troops ) and unit.energy >= 10) return false;			

       if (!pk::is_player_controlled(unit) and (int(unit.troops) < 극소병력부대_복귀병력기준 or unit.food < float (부대복귀_병량기준_비율 * unit.troops ) or ((weapon_type == 2 or weapon_type == 3) and unit.energy < 10)))   
          {				
            pk::person@ leader = pk::get_person(unit.leader);
            ...
            for (int i = 0; i < int(arr.length); i++)
            {
                ...
                if (pk::is_alive(base))
                {
                   int base_id  = base.get_id();					  
                    if (unit.get_force_id() == base.get_force_id() and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_거점끝)  
                    {            
                       if (450 < base.hp and pk::get_food(base) >= 2000 and pk::get_food(base) / float(pk::get_troops(base) + 1) >= 1.5f)
                       {
						 if (건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and pk::get_troops(base) + unit.troops <= pk::get_max_troops(base) * 진입부대_초과한도기준)   
                           dst_list.add(base);  // 전방 리스트 추가	
					   
						 if (건물_관문시작 <= base.get_id() and base.get_id() < 건물_거점끝 and pk::get_troops(base) + unit.troops <= pk::get_max_troops(base) * 항관진입_초과한도기준)   
                           dst_list.add(base);  					   
                       }
            ...
            // 중간이동지점에 적의 거점을 지나가야 하면 중단, 전방 적의 침입 발생 도시와 중간이동지점도시 사이의 거리가 1 초과하고 현재 부대가 있는 도시와 중간 이동 지점 도시 간 거리가 1 초과하면 중단
            for (int i = 0; i < int(paths.length); i++)
            {
                ...
                if (arr_target_base_id[unit_id] != service_id)
                {
                    ...
                    if (대화표시설정 and pk::is_in_screen(unit.pos))
                    {
                        ...
                        switch(pk::rand(3))
                        {
                            case 0: pk::say(pk::encode(pk::format("지금은 부대 존속조차 어렵다. \n원 소속지인 \x1b[2x{}\x1b[0x(으)로 간다!", target_name)), leader); break;
                ...
                if (arr_target_base_id[unit_id] != target_id)
                {
                    ...
                    // 아군 거점 이동 재설정
                    pk::set_order(unit, 부대임무_공격, Friendly_base.pos);
                    ...
                    if (대화표시설정 and pk::is_in_screen(unit.pos))
                    {
                        ...
                        switch(pk::rand(3))
                        {
                            case 0: pk::say(pk::encode(pk::format("지금은 부대 존속조차 어렵다. \n그나마 가까운 \x1b[2x{}\x1b[0x(으)로 간다!", target_name)), leader); break;
                            case 1: pk::say(pk::encode(pk::format("지금은 부대 존속조차 어렵다. \n그나마 가까운 \x1b[2x{}\x1b[0x(으)로 간다!", target_name)), leader); break;
                            case 2: pk::say(pk::encode(pk::format("지금은 부대 존속조차 어렵다. \n그나마 가까운 \x1b[2x{}\x1b[0x(으)로 간다!", target_name)), leader); break;
                        }
                    }
     ...
     // 거점병력 부족시 부대 수성 명령, 2021.01.15 일송정 수정
        bool defend_castle (pk::unit@ unit)
        {
            ...
            for (int i = 0; i < int(arr.length); i++)
            {
                ...
                // 거점 병력 부족 조건 : 1) 거점 주변에 적이 있는데 거점 병력이 7천 5백이하. 2) 적부대가 멀리 있으면 거점 병력이 5천 이하인 경우.  3)적이 없어도 병력 2천5백 이하인 경우
				// 항관 병량 부족 조건 : 부대 병량 5천 이상에 부대병력보다 많은데, 거점 병량이 5천 이하에 거점병력 * 0.4보다 적을때
				// 항관 금 부족 조건 : 부대 소지 금은 500 이상, 거점 내 금 보유량이 500 이하
                // 거점 내구도 심각하게 떨어진 수준이면 제외 2021.09.07 일송정 수정	

                if (unit.get_force_id() != base.get_force_id()) return false;
                if (건물_파양항 == base_id or 건물_노릉항 == base_id or 건물_서하항 == base_id or 건물_하양항 == base_id or 건물_해현항 == base_id or 건물_강도항 == base_id) return false; 
                if (450 >= base.hp) return false;			

                    if (건물_도시시작 <= base_id and base_id < 건물_거점끝
						...
						and 450 < base.hp 
						and func_already_base_dist(base, (base.facility == 시설_도시) ? 2 : 5 ) <= 1
						and 건물_파양항 != base_id and 건물_노릉항 != base_id and 건물_서하항 != base_id and 건물_하양항 != base_id and 건물_해현항 != base_id and 건물_강도항 != base_id) 
                        {
                           dst_list.add(base);  
                        }

                    else if (건물_관문시작 <= base_id and base_id < 건물_거점끝
                        and unit.get_force_id() == base.get_force_id() 
            ...
            {				
                ...
                // 아군 거점으로 이동
                if (arr_target_base_id[unit_id] != target_id or unit.order != 부대임무_공격)
                {
                    // 목표 재설정 정보 저장
                    arr_target_base_id[unit_id] = target_id;
                    // 아군 거점 이동 재설정
		...
		int getBaseToRescue(pk::unit@ unit)
		{
            ...
            for (int i = 0; i < int(arr.length); i++)
            {
                ...
                // 거점 병력 부족 조건 : 1) 거점 주변에 적이 있는데 거점 병력이 8천 이하. 2) 부대들이 직접 대면 교전 중이거나 거점과 주변 병력 총합이 적에게 밀리는 상황에서 거점 병력이 7천 이하. 
				// 항관 병량 부족 조건 : 부대 병량 5천 이상에 부대병력보다 많은데, 거점 병량이 5천 이하에 거점병력 * 0.4보다 적을때
				// 항관 금 부족 조건 : 부대 소지 금은 500 이상, 거점 내 금 보유량이 500 이하
// 거점 내구도 심각하게 떨어진 수준이면 제외 2021.09.07 일송정 수정				
                    ...
                    if (건물_도시시작 <= base.get_id() and base.get_id() < 건물_거점끝
                        ...
                        {
                           best_dst = base_id;
                           dst_list.add(base);  // 수송가능 거점리스트 추가
                        }

                    else if (건물_관문시작 <= base.get_id() and base.get_id() < 건물_거점끝
                        and unit.get_force_id() == base.get_force_id() 
                        and ((unit.food >= 5000 and unit.food >= unit.troops and (pk::get_food(base) <= 5000 and pk::get_food(base) <= pk::get_troops(base) * 0.4f) )
						 or (unit.gold >= 500 and pk::get_gold(base) <= 500))					
                        and 450 < base.hp and func_already_base(base) <= 1
        ...
        bool front_base_retreat(pk::unit@ unit)
        {
			...
			// 병력이나 병량이 바닥난 부대들은 제외
            if (int(unit.troops) < 극소병력부대_복귀병력기준 or unit.food < float (부대복귀_병량기준_비율 * unit.troops )) return false;
            
            int candidate_count = list_candidate_unit.count;
            if (candidate_count > 0) return false;
 
			// 아군거점으로 향하고 있는 부대는 제외
            pk::building@ unit_target_base = pk::get_building(unit.target);
            if (pk::is_alive(unit_target_base) and unit_target_base.get_id() < 건물_거점끝 and unit.get_force_id() == unit_target_base.get_force_id() and unit.order == 부대임무_공격) return false;  
      ...
      if (pk::is_alive(pk::get_building(target)) and neighbor_city_base(pk::get_building(target), service_base))
       {
			...
			// 전방거점 범위 내 적이 없으면 후퇴 안함
              /*  // if (!enemy_approach(pk::get_building(target))) return false; 사용했었음. enemy_around_distance(target_base, 6) 대신 */				
			if (건물_도시시작 <= pk::get_building(target).get_id() and pk::get_building(target).get_id() < 건물_도시끝 and !enemy_around_distance(pk::get_building(target), 7)) return false;
			if (건물_관문시작 <= pk::get_building(target).get_id() and pk::get_building(target).get_id() < 건물_거점끝 and !enemy_around_distance(pk::get_building(target), 4)) return false;			
         ...
         if (0 < ilban_list.length)
         {
            ...
            for (int i = 0; i < int(paths.length); i++)
            {
                ...
                if (hex.has_building)
                   ...
                   if (pk::is_alive(base_p) and base_p.get_id() < 건물_거점끝 and base_p.get_id() != pk::get_building(target).get_id() and pk::is_enemy(base_p, unit)) return false;
            ...
            auto paths_TargetFriendly = pk::get_path(unit, pk::get_building(target).get_pos(), pk::get_building(targetFriendly).get_pos());
                    ...
                    {
	        // 	현재 부대가 있는 아군거점에 적의 침입이나 접근전이 있으면 후퇴 안함.	
              /*  // if (enemy_approach(unit_base)) return false; 사용했었음. if (enemy_around_distance(unit_base, 6)) return false; 대신 */					
			if (enemy_around_distance(unit_base, 6)) return false;
			if (close_combat(unit_base)) return false;

                    // 현 부대와 전방 전투 지역간 거리, 전방 전투 지역과 인근 아군 거점 간의 거리 비교
                   ...
                   { 
                      ...
                      {   
                        ...
                        arr_target_base_id[unit_id] = target_id;
                        // 아군 거점 이동 재설정					
                        pk::set_order(unit, 부대임무_공격, targetFriendly_base.get_pos());
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                    ...
                    // 전방 전투가 벌어진 지역과 인근 아군 거점 간의 거리 비교
                   ...
                   { 
                      ...
                      {   
                        ...
                        arr_target_base_id[unit_id] = target_id;
                        // 아군 거점 이동 재설정					
                        pk::set_order(unit, 부대임무_공격, target_base.get_pos() );
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                            string front_base_name = pk::decode(pk::get_name(target_base));
  ...
  for (int f = 0; f < 건물_거점끝; f++)
   {
             ...
             if ( 건물_도시시작 <= front_base.get_id() and front_base.get_id() < 건물_거점끝)
	{
		...
		/** 적의 거점으로부터 떨어진 후방거점들 */
              /*  //  and enemy_approach(front_base) 사용했었음. enemy_around_distance(front_base, 6) 대신 */	

            if (11 <= pk::get_distance(front_base.pos, unit.pos) and enemy_around_distance(front_base, 6) and ((front_base_id < 건물_도시끝 and troops_already(front_base) + (( a - 1 ) * 6000) <= gap_troops(front_base)) or (front_base_id >= 건물_도시끝 and troops_already(front_base) + (Friendly_base(front_base) * 0.5f) <= gap_troops(front_base))) and 450 < front_base.hp and 0 < enemy_weight )
                   {
	...
	return false;
        ...
        void search_enemy_on_reinforce_engage(pk::unit@ unit)
        {
            ...
            if (unit.target_type == 부대임무대상_거점)
            {
                ...
                if (pk::is_alive(base_t))
                {
                    ...
                    {
                        pk::array<pk::point> range = pk::range(base_p.pos, 1, 10);  // 거점 주변 검색
                        for (int i = 0; i < int(range.length); i++)
                        {
        ...
        void func_change_target_unit(pk::unit@ unit)
        {
			...
			// 목표 거점과의 거리 5 이상의 정복 임무의 부대는 제외
            if (pk::is_alive(unit_target_base) and 건물_도시시작 <= unit_target_base.get_id() and unit_target_base.get_id() < 건물_거점끝 and pk::is_enemy(unit_target_base, unit)
				and unit.order == 부대임무_정복) return;  

			// 병량 수송 목적의 아군거점으로 향하고 있는 부대는 제외
            if (pk::is_alive(unit_target_base) and 건물_도시시작 <= unit_target_base.get_id() and unit_target_base.get_id() < 건물_거점끝 and unit.get_force_id() == unit_target_base.get_force_id() 
				and unit.order == 부대임무_공격 and unit.food >= 6500 and unit.troops < 5000) return;  

			// 병력이나 병량이 바닥난 부대들은 제외
            if (int(unit.troops) < 극소병력부대_복귀병력기준 or unit.food < float (부대복귀_병량기준_비율 * unit.troops )) return;

			// 병영 공격이 목표인 부대들은 제외
            if (pk::is_alive(unit_target_base) and (unit_target_base.facility == 시설_병영 or unit_target_base.facility == 시설_병영2단 or unit_target_base.facility == 시설_병영3단)) return;

			// 붕괴 위험 있는 아군 거점 혹은 병력 지원 필요한 아군 거점 근처의 부대들은 제외
            pk::array<pk::point> arr = pk::get_movable_pos(unit);
            for (int i = 0; i < int(arr.length); i++)
            {
              pk::building@ base = pk::get_building(arr[i]);
              ...
              {	
                if (건물_관문시작 <= base.get_id() and base.get_id() < 건물_거점끝 and base.hp <= 450)
			    return;	
                if (건물_도시시작 <= base.get_id() and base.get_id() < 건물_거점끝 and pk::get_troops(base) <= 7500)
			    return;	
			    }
			  }
			}	
        ...
        //  계속 공격					
             int a = pk::get_person(pk::get_kunshu_id(service_base)).character;
              ...
              /*  // if (enemy_approach(service_base)) return false; 사용했었음. enemy_around_distance(front_base, 6) 대신 */							
            ...
            for (int i = 0; i < int(paths.length); i++)
            {
                ...
                if (!pk::is_enabled_terrain(hex.terrain)) return false;
                    ...
                    if (arr_target_base_id[unit_id] != target_id)
                    {
                        ...
                        arr_target_base_id[unit_id] = target_id;
                        // 적지이면 적거점 공격 재설정
                        pk::set_order(unit, 부대임무_정복, pk::get_building(target).get_pos());
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
		...
		/** 승산이 충분한 진격할만한 적의 거점 찾기 (2022.06. 일송정 추가) */
        ...
        //  계속 공격					
                    ...
                    {												
                       if (건물_파양항 != target_id and 건물_노릉항 != target_id and 건물_서하항 != target_id and 건물_하양항 != target_id and 건물_해현항 != target_id and 건물_강도항 != target_id)
                           {								
                           best_dst = target_id;
                           dst_list.add(target_base);  // 전방 리스트 추가
	                       }					   
  ...
  for (int f = 0; f < 건물_거점끝; f++)
   {
		...
		int getAnotherEnemyBase(pk::unit@ unit)
		{
            ...
            pk::building@ unit_target_base = pk::get_building(unit.target);	
          if (pk::is_alive(unit_target_base) and unit_target_base.get_id() < 건물_거점끝 and pk::is_enemy(unit_target_base, unit))
          {  
            ...
            int adjacent_base_id = adjacent_base.get_id();				 
                ...
                if (unit_base_id != adjacent_base_id and unit_target_base_id != adjacent_base_id and 건물_도시시작 <= adjacent_base_id and adjacent_base_id < 건물_거점끝)
                 {            
	 ...
	 return false;
        ...
        bool atk_enemy_base_nearby(pk::unit@ unit)
        {
			...
			// 이미 아무 거점을 타겟 삼은 부대들은 제외 (병목현상시 우회하는 부대들 위해)	and pk::is_enemy(unit_target_base, unit)	
            pk::building@ unit_target_base = pk::get_building(unit.target);
            if (pk::is_alive(unit_target_base) and unit_target_base.get_id() < 건물_거점끝) return false;  
            ...
            if (pk::is_alive(service_base) and pk::is_alive(target_base))
            {
                ...
                if (is_enemy_base and is_candidate_base and (arr_target_base_id[unit_id] != target_id and arr_target_unit_id[unit_id] != -1) )
                {
                    if ((적거점_점령성격적용 and (leader.character == 성격_대담 or leader.character == 성격_저돌)) or !적거점_점령성격적용)
                    {
                        // 목표 재설정 정보 저장
                        arr_target_base_id[unit_id] = target_id;
                        // 적지이면 적거점 공격 재설정 (2024.05. 공격에서 정복으로 일송정 수정)
        ...
        bool no_holds_barred(pk::unit@ unit)
        {
            ...
            for (int i = 0; i < int(paths.length); i++)
            {
                ...
                if (hex.has_building)
                   {
                   pk::building@ base_p = pk::get_building(pk::get_building_id(paths[i]));
                   if (pk::is_alive(base_p) and base_p.get_id() < 건물_거점끝 and base_p.get_id() != enemy_base_id and pk::is_enemy(base_p, unit)) return false;
                   }
            ...
            if (pk::is_alive(empty_base) and pk::is_alive(unit_base))
            {
                ...
                if (건물_파양항 == empty_base_id or 건물_노릉항 == empty_base_id or 건물_서하항 == empty_base_id or 건물_하양항 == empty_base_id or 건물_해현항 == empty_base_id or 건물_강도항 == empty_base_id )
                    ...
                    if ((pk::is_neighbor_base(empty_base_id, unit_base_id) or empty_base_id == unit_base_id)
						and 건물_파양항 != empty_base_id and 건물_노릉항 != empty_base_id and 건물_서하항 != empty_base_id and 건물_하양항 != empty_base_id and 건물_해현항 != empty_base_id and 건물_강도항 != empty_base_id)
                           {								
        ...
        bool attack_barrack(pk::unit@ unit)
        {
            ...
            pk::building@ unit_target_base = pk::get_building(unit.target);
            if (pk::is_alive(unit_target_base) and (unit_target_base.facility == 시설_병영 or unit_target_base.facility == 시설_병영2단 or unit_target_base.facility == 시설_병영3단)) return false;
            ...
            if (pk::is_alive(service_base) and pk::is_alive(unit_base))
            {
                if (unit_base.facility != 시설_도시) return false;				
                ...
                if (pk::is_enemy(unit, unit_base) and pk::is_alive(unit) and pk::is_alive(unit_base))
                {
                   ...
                   if (!pk::has_facility(city, 시설_병영) and !pk::has_facility(city, 시설_병영2단) and !pk::has_facility(city, 시설_병영3단) )  return false;	
	 ...
	 int func_already_empty(pk::building@ building)
        ...
        bool unit_base_nearby(pk::unit@ unit)
        {
			...
			for (int i = 0; i < int(range.length); i++)
			{
				...
				if (pk::is_alive(unit_k))
				{
					...
					if (pk::is_enemy(target_base, unit_k) and unit_k.type == 부대종류_전투 and ( unit_k.weapon == 병기_충차 or unit_k.weapon == 병기_정란 or unit_k.weapon == 병기_투석 or unit_k.weapon == 병기_목수 ) )
					{
						if (distance <= 1)
						{
							Siege_weapon_units1++;
					...
					else if (target_base.get_force_id() == unit_k.get_force_id() and unit_k.type == 부대종류_전투)
					{
                        if (distance <= 7)
						{
							enemy_units3++;
					...
					else if (pk::is_enemy(target_base, unit_k) and unit_k.type == 부대종류_전투 and ( unit_k.weapon == 병기_군마 or unit_k.weapon == 병기_창 or unit_k.weapon == 병기_극 or unit_k.weapon == 병기_노 or unit_k.weapon == 병기_검))
					{
                        if (distance <= 14)
						{
                    ...
                    {						
                        ...
                        pk::set_order(unit, 부대임무_정복, pk::get_building(target).get_pos()) : pk::set_order(unit, 부대임무_공격, pk::get_building(target).get_pos());
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                            string target_name = pk::decode(pk::get_name(pk::get_building(target)));
						  if (pk::is_enemy(unit, pk::get_building(target)))								
                          {
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("여긴 공성부대들에게 맡기고\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("여긴 공성부대들에게 맡기고\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("여긴 공성부대들에게 맡기고\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                            }								
                          }	
						  else
                          {
			...
			for (int i = 0; i < int(range.length); i++)
			{
				...
				{
					int distance = pk::get_distance(target_base.get_pos(), pos);
					if (pk::is_enemy(target_base, unit_k) and unit_k.type == 부대종류_전투 and ( unit_k.weapon == 병기_주가 or unit_k.weapon == 병기_누선 or unit_k.weapon == 병기_투함 ) )
					{
						if (distance <= 1)
						{
							Sea_weapon_units1++;
					...
					else if (target_base.get_force_id() == unit_k.get_force_id() and unit_k.type == 부대종류_전투 and ( unit_k.weapon == 병기_주가 or unit_k.weapon == 병기_누선 or unit_k.weapon == 병기_투함 ) )
					{
                        if (distance <= 7)
						{
							enemy_units3++;
			...
			if (EnemyPort == -1) 
            {
                    ...
                    // 인근 침공할만한 적의 항구는 없고 다른 적의 거점이 있다면	
                    if (arr_target_base_id[unit_id] != pk::get_building(target).get_id())
                      {
                        // 목표 재설정 정보 저장
                        arr_target_base_id[unit_id] = pk::get_building(target).get_id();						
                        ...
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
						  ...
						  if (pk::is_enemy(unit, pk::get_building(target)))								
                          {
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("수상전에선 그냥 죽치고 있으면 안된다.\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("수상전에선 그냥 죽치고 있으면 안된다.\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("수상전에선 그냥 죽치고 있으면 안된다.\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                            }								
                          }	
						  else
                          {							
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("수상전에서 이렇게 가만히 있을 바엔\n우리는 \x1b[2x{}\x1b[0x(으)로 돌아가자.", target_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("수상전에서 이렇게 가만히 있을 바엔\n우리는 \x1b[2x{}\x1b[0x(으)로 돌아가자.", target_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("수상전에서 이렇게 가만히 있을 바엔\n우리는 \x1b[2x{}\x1b[0x(으)로 돌아가자.", target_name)), leader); break;
                            }
                          }							
                        }	 										 
                      }		  
                    ...
                    if (arr_target_base_id[unit_id] != pk::get_building(EnemyPort).get_id() and target_id != pk::get_building(EnemyPort).get_id() and unit.troops >= 4000 )
                      {
                        ...
                        // 목표 거점 재설정							  
                        pk::set_order(unit, 부대임무_정복, pk::get_building(EnemyPort).pos);
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
			...
			for (int i = 0; i < int(range.length); i++)
			{
				...
				{
					int distance = pk::get_distance(target_base.get_pos(), pos);
					if (pk::is_enemy(target_base, unit_k) and unit_k.type == 부대종류_전투 and ( unit_k.weapon == 병기_충차 or unit_k.weapon == 병기_정란 or unit_k.weapon == 병기_투석 or unit_k.weapon == 병기_목수 or unit_k.weapon == 병기_주가 or unit_k.weapon == 병기_누선 or unit_k.weapon == 병기_투함) )
					{
						if (distance <= 4)
						{
							enemy_units1++;
                    ...
                    {						
                        ...
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
						  ...
						  if (pk::is_enemy(unit, pk::get_building(target)))								
                          {
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("성을 지켜줄 아군 세력은 이미 충분하다!\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("성을 지켜줄 아군 세력은 이미 충분하다!\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("성을 지켜줄 아군 세력은 이미 충분하다!\n우리는 \x1b[2x{}\x1b[0x(으)로 진격하자!", target_name)), leader); break;
                            }								
                          }	
						  else
                          {
        ...
        bool unit_jam(pk::unit@ unit)
        {
            ...
            if (unit.target == pk::get_building(target).get_id()) return false;	
              ...
              /*  //  and enemy_approach(target_base))사용했었음. enemy_around_distance(target_base, 6) 대신 */                    
                    ...
                    {						
                        ...
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
						  ...
						  if (pk::is_enemy(unit, pk::get_building(target)))								
                          {
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("대치 상태에 죽치고 있을 바엔\n우리는 차라리 \x1b[2x{}\x1b[0x(으)로 진격한다!", target_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("대치 상태에 죽치고 있을 바엔\n우리는 차라리 \x1b[2x{}\x1b[0x(으)로 진격한다!", target_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("대치 상태에 죽치고 있을 바엔\n우리는 차라리 \x1b[2x{}\x1b[0x(으)로 진격한다!", target_name)), leader); break;
                            }								
                          }	
						  else
                          {
  ...
  for (int f = 0; f < 건물_거점끝; f++)
   {
             ...
             if ( 건물_도시시작 <= adjacent_base.get_id() and adjacent_base.get_id() < 건물_거점끝)
	{
		...
		/** 적의 거점으로부터 떨어진 후방거점들 */
            ...
            if (dst_list.count == 0)
                best_dst = -1;
            else if (int(main.unit_k.troops) >= 극소병력부대_복귀병력기준)
            {
                dst_list.sort(function(a, b)
                {
                bool enemies_around_a = (pk::enemies_around(a));
	 ...
	 int jam_f_units_around(pk::unit@ unit)
		{
			...
			for (int i = 0; i < int(range.length); i++)
			{
				...
				if (pk::is_alive(unit) and pk::is_alive(unit_around))
				{
					...
					if (unit.get_force_id() == unit_around.get_force_id() and unit.type == 부대종류_전투 and unit_id != unit_around_id)
					{
                        if (distance <= 4)
						{
							force_units1++;
	 ...
	 int check_unit_bound(pk::building@ base)
	{
        ...
        pk::array<pk::point> range = pk::range(base.pos, 1, 1);  // 거점 주변 검색																   						 
        for (int i = 0; i < int(range.length); i++)
        {
            pk::point pos_i = range[i];
            pk::unit@ unit = pk::get_unit(pos_i);
            if (pk::is_alive(unit) and unit.get_force_id() == base.get_force_id() and unit.target == base.get_id() and 3000 <= unit.troops and unit.order == 부대임무_공격 and pk::get_distance(base.pos, unit.pos) <= 1)
			{					
				units++;
            }
	 ...
	 {
			int already = 0;
            pk::array<pk::point> range = pk::range(building.pos, 1, distance);  // 거점 주변 검색																   						 
            for (int i = 0; i < int(range.length); i++)
            {
              pk::point pos_i = range[i];
              pk::unit@ unit = pk::get_unit(pos_i);
			...
			if (unit.order != 부대임무_공격) continue;	
			if (unit.target != building.get_id()) continue;			
																												    
		    if ( pk::is_alive(unit) and 3000 <= unit.troops  and pk::is_enemy(unit, building) and unit.target == building.get_id() and unit.order == 부대임무_공격) 
	 ...
	 int func_already_base(pk::building@ building)
	 {
        ...
        //---------------------------------------------------------------------------------------
           ...
           // 해당 거점을 향해 이미 출진한 상태인 그 거점 거리별 병력 3천 이상의 부대 숫자 체크,  masterpiecek님의 AI도시물자수송을 참조했습니다. 2021.09.02 일송정 추가
	...
	return false;			
	 ...
	 int defensive_around_base(pk::building@ building, int distance)
        ...
        bool close_combat(pk::building@ base)
        {
                ...
                pk::array<pk::point> range = pk::range(base.pos, 1, 10);  // 거점 주변 검색																   						 
                for (int i = 0; i < int(range.length); i++)
                {
                            pk::point pos_i = range[i];
	...
	return false;
        ...
        bool retreat_exceed_unit(pk::unit@ unit)
        {
            ...
            if (!pk::enemies_around(pos_area_base))
            {
             ...
             // 병력 한도 초과하는 경우 (2024.03.13 일송정 수정)
             if (is_exceed_unit and (arr_target_base_id[unit_id] == pos_area_id and arr_target_unit_id[unit_id] != -1))
             {

                // 현재 위치 인근에 적의 거점이 없거나 부대 상태가 온전치 못하면 소속 거점으로 퇴각
                if (enemy_weight == 0 or int(unit.troops) < 극소병력부대_복귀병력기준 or unit.food < float (부대복귀_병량기준_비율 * unit.troops ))
                {
                    // 목표 재설정 정보 저장
                    arr_target_base_id[unit_id] = service_id;
                    // 소속 거점으로 돌아감
                    ...
                    if (대화표시설정 and pk::is_in_screen(unit.pos))
                    {
                        ...
                        switch(pk::rand(3))
                        {
                            case 0: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x(으)로 퇴각하라", old_order, target_name)), leader); break;
                ...
                // 부대 상황이 괜찮고 현재 위치에 공격할만한 적의 거점이 있으면 진격
                if (enemy_weight > 0 and int(unit.troops) >= 극소병력부대_복귀병력기준 and unit.food >= float (부대복귀_병량기준_비율 * unit.troops ))
                {
					// 진격할 적의 거점 찾기
					int target_enemy = getAnyEnemyBase(unit);
                    pk::building@ target_enemy_base = pk::get_building(target_enemy);

                  if (pk::is_alive(target_enemy_base) and target_enemy_base.get_id() < 건물_거점끝)
                  {	
                    int target_enemy_base_id = target_enemy_base.get_id();			  
                    // 목표 재설정 정보 저장
                    arr_target_base_id[unit_id] = target_enemy_base_id;
                    // 소속 거점으로 돌아감
                    pk::set_order(unit, 부대임무_정복, target_enemy_base.pos);
                    // 목표변경 메시지
                    if (대화표시설정 and pk::is_in_screen(unit.pos))
                    {
                        string old_order = pk::format("\x1b[2x{}\x1b[0x은 가지 않아도 된다\n", pk::decode(pk::get_name(pos_area_base)));
                        ...
                        switch(pk::rand(3))
                        {
                            case 0: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x을(를) 노린다!", old_order, target_name)), leader); break;
                            case 1: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 진격한다!", old_order, target_name)), leader); break;
                            case 2: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x(으)로 공격한다!", old_order, target_name)), leader); break;
                        }
                    }
                    if (디버그모드) pk::info(pk::format("부대의 새로운 목표거점: {}", pk::decode(pk::get_name(service_base))) );
        ...
        // 정복 임무 부대들이 거점을 차지한 이후 계속 진군할 것인지, 거점으로 들어갈 것인지
        // 부대임무 정보 조회
        bool is_order_retreat = (unit.order == 부대임무_퇴각 or (unit.order == 부대임무_물림 and candidate_count == 0));
		
        if (unit.order == 부대임무_공격 or unit.order == 부대임무_정복 or is_order_retreat)
        {
            if (!pk::is_alive(unit)) return false;
																				   			
            pk::building@ unit_target_base = pk::get_building(unit.target);
            if (!pk::is_alive(unit_target_base)) return false;   
            if (unit_target_base.get_id() >= 건물_거점끝) return false;   
            ...
            bool is_in_movable_pos = check_base_in_movable_range(unit, pos_area_base); // 부대 이동거리 내에 목표거점이 존재하는 경우
            bool is_in_city_area   = (pk::get_city_id(pos_area_base.pos) == pk::get_city_id(unit.pos));  // 부대위치가 목표거점과 같은 도시영역인 경우
            bool is_force_base     = (unit.get_force_id() == pos_area_base.get_force_id());
         ...
         if (pk::is_alive(unit_target_base) and unit_target_base.get_id() < 건물_거점끝 and unit.get_force_id() == unit_target_base.get_force_id() and unit_target_base.get_id() == pos_area_id)
         {
            // 현재 위치 근처에 적의 거점이 있고, 부대 상태 괜찮으면 진격
            if (int(unit.troops) >= 극소병력부대_복귀병력기준 and unit.food >= float (부대복귀_병량기준_비율 * unit.troops ))
            {
				// 진격할 적의 거점 찾기
				int target_enemy = getAnyEnemyBase(unit);
                pk::building@ target_enemy_base = pk::get_building(target_enemy);				

                if (pk::is_alive(target_enemy_base) and pk::is_alive(target_enemy_base) and target_enemy_base.get_id() < 건물_거점끝)
                {					
                    ...
                    if (디버그모드) pk::info(pk::format("---{}대 {}:{}", unit_name, get_order_info(unit.order), target_enemy_base_name));
                    
                    // 적의 거점 인지하였고, 현재 위치 아군 거점이 꽉 차있다면
                    ...
                    { 
                      if (arr_target_base_id[unit_id] != target_enemy_base_id)
                      {
                        ...
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                            string old_order = pk::format("들어라! \x1b[2x{}\x1b[0x(은)는 우리 군의 영토다!\n", pos_area_base_name);

                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x(을)를 노린다!", old_order, target_enemy_base_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 진격한다!", old_order, target_enemy_base_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x(을)를 공격한다!", old_order, target_enemy_base_name)), leader); break;
                            }
                        }
                        if (디버그모드) pk::info(pk::format("새로운 부대 공격 목표거점: {}", target_enemy_base_name) );
                      }
                    }
                }
            }
                    ...
                    { 
                      if (arr_target_base_id[unit_id] != pos_area_id)
                      {
                        ...
                        // 부대임무 재설정 : 새로운 목표 거점으로 이동
                        pk::set_order(unit, 부대임무_공격, pos_area_base.pos);
                        // 목표변경 메시지
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                            string old_order = pk::format("들어라! \x1b[2x{}\x1b[0x(은)는 우리 군의 영토다!\n", pos_area_base_name);
                            ...
                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 입성한다!", old_order, pos_area_base_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 입성한다!", old_order, pos_area_base_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 입성한다!", old_order, pos_area_base_name)), leader); break;
                            }
                        }
                        if (디버그모드) pk::info(pk::format("부대 이동 목표거점 갱신: {}", pos_area_base_name) );
                      }
                    }		 
         ...
         if (is_order_retreat or unit.target_type == 부대임무대상_거점)
         {
            ...
            if (enemy_weight > 0 and int(unit.troops) >= 극소병력부대_복귀병력기준 and unit.food >= float (부대복귀_병량기준_비율 * unit.troops ))
            {
                ...
                pk::building@ target_enemy_base = pk::get_building(target_enemy);				
                pk::building@ base_t = (is_order_retreat)? service_base : pk::get_building(unit.target);
                if (pk::is_alive(base_t) and base_t.get_id() < 건물_거점끝 and unit.get_force_id() == base_t.get_force_id() and pk::is_alive(target_enemy_base) and target_enemy_base.get_id() < 건물_거점끝)
                {
                    // 거점진입 대상 거점의 부대진입 시 병력 한도 확인
                    string base_t_name     = pk::decode(pk::get_name(base_t));
                    int target_id          = base_t.get_id();
                    ...
                    { 
                        ...
                        if (대화표시설정 and pk::is_in_screen(unit.pos))
                        {
                            string old_order = pk::format("\x1b[2x{}\x1b[0x은 더 이상 우리의 목적지가 아니다!\n", base_t_name);

                            switch(pk::rand(3))
                            {
                                case 0: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x을(를) 노린다!", old_order, target_enemy_base_name)), leader); break;
                                case 1: pk::say(pk::encode(pk::format("{}우리는 \x1b[2x{}\x1b[0x(으)로 진격한다!", old_order, target_enemy_base_name)), leader); break;
                                case 2: pk::say(pk::encode(pk::format("{}\x1b[2x{}\x1b[0x을(를) 공격한다!", old_order, target_enemy_base_name)), leader); break;
                            }
                        }
                        if (디버그모드) pk::info(pk::format("새로운 부대 공격 목표거점: {}", target_enemy_base_name) );
                    }
        ...
        bool check_exceed_unit(pk::building@ base, pk::unit@ unit, bool isforce=true)
        {
            ...
            if (unit.type == 부대종류_전투)
            {
                ...
                {
                // 병량이 부족한 부대는 예외 기준설정, 2024.03.06 일송정 수정
                // 부대 병량이 부족해서 병력대비 0.5배 이하시
	                if (unit.food <= unit.troops * 0.5f)			
                    exceed_limit  = pk::max(0.0f, pk::min(1.0f, 도시_병량부족_부대_초과한도기준));	
                ...
                if (진입부대_초과병력귀환 and base.get_id() >=  건물_도시끝)
                {
                // 병량이 부족한 부대의 항구와 관문 한도초과 기준설정, 2024.03.06 일송정 수정
                // 부대 병량이 부족해서 병력대비 0.5배 이하시				
                 if ((unit.food <= unit.troops * 0.5f and pk::get_food(base) >= pk::get_troops(base) * 0.7f))
                 exceed_limit2  = pk::max(0.0f, pk::min(1.0f, 항관_병량부족_부대_초과한도기준));

                // 병량이 부족한 항구와 관문 한도초과 기준설정, 2024.03.06 일송정 수정
				// 거점에 도움될 병량 충분한 부대가 있을 경우 (병력 5천 미만 소규모 부대)
				// 거점 병량이 부족, 병력대비 0.4배 이하여서 인근 부대의 병량이 급하게 필요할 경우			 
                 ...
                 exceed_limit2  = pk::max(0.0f, pk::min(1.0f, 병량부족_거점_초과한도기준));
