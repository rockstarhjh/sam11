...
namespace AI인재등용_AI_Recuite_v2
{
	...
	const int AI_재야등용_주기 = 20; //10일에 한번

	const bool AI_사관등용 = false;// 2달연속으로 같은도시에 있는경우, 자동으로 해당도시에 등용됨.(무장행동 소모 없음)
	const int AI_사관등용_금지기간 = 360; //시작후 1년동안 사관등용을 하지 않습니다.

	const bool AI_포로자동등용 = false;//포로가 충성도가 90미만이 되면 자동으로 등용됩니다.(무장행동 소모 없음)
	const int AI_포로자동등용_충성도 = 75;//AI간의 포로일때,80미만인경우 자동등용됨.
	const int AI_포로자동등용_충성도_유저 = 85;//유저무장이 AI의 포로일때, 90미만인경우 자동등용됨.
	...
	const bool AI_재야자동탐색 = false;//AI재야탐색을 하지 않아도 랜덤으로 탐색됩니다.(무장행동 소모 없음)
	...
	const bool 인재등용_AI유리 = false;
	const int 유저_인재등용_부가수치 = -5;//유저가 AI무장 등용할때,30%확률을 더 뺌(어렵게) 
	const int AI_인재등용_부가수치 = 5;//AI가 유저 무장 등용할때 30%확률을 더함(쉽게)
