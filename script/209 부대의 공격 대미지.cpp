namespace 부대의_공격_대미지
{
	class Main
	{
		...
		void callback(pk::damage_info& info, pk::unit@ attacker, int tactics_id, const pk::point &in target_pos, int type, int critical, bool ambush)
		{
			...
			else if (pk::is_neighbor_pos(attacker.get_pos(), target_pos) or ambush)
			{
				troops_atk = 10;
			}
			...
			if (target_unit !is null)
			{
				...
				else
				{
					...
					else
					{
						...
						// 기습 기교 효과 추가 (특기종합패치)
						else if (target_unit.has_tech(기교_기습) and (target_unit.weapon == 병기_창) and (pk::get_hex(target_unit.get_pos()).terrain == 지형_숲))
							troops_damage = 0;
					}
				}

				// 등갑이 방사 공격은 반으로 줄이지 않도록 변경 (특기종합패치)
				if (target_unit.has_skill(특기_등갑) and tactics_id != 전법_공성방사)
					troops_damage = troops_damage / 2;

				// 답파 특기가 투석 전법에 절반만 피해를 받음 (특기종합패치)
				if (target_unit.has_skill(특기_답파) and (tactics_id == 전법_공성투석 or tactics_id == 전법_함선투석))
					troops_damage = troops_damage / 2;
