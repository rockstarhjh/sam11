...
namespace 매월_인사이동utf8
{
	...
	class Main
	{
		...
		bool callback(pk::force@ force)
		{
                ...
                if (!force.is_player() and pk::is_normal_force(force_id) and pk::get_elapsed_months() > 3)
			{
                    ...
                    if (kunshu_base.get_force_id() == force_id and (pk::get_month() == 1 or pk::get_month() == 7) and (pk::get_day() == 1 or pk::get_day() == 21))
                        Pushkunshu(kunshu_base);               //  세력 도시 5개 이상이면 군주를 후방 도시로
        ...
        bool Pushkunshu(pk::building@ base)
        {
            ...
            // 군주 미행동이고 부재중이 아니어야
            if (pk::is_unitize(kunshu)) return false;			
            if (kunshu.action_done) return false;
            if (pk::is_absent(kunshu)) return false;
            if (kunshu.order != -1) return false;
            if (kunshu.mibun == 신분_포로) return false;
            if (kunshu.mibun == 신분_재야) return false;		
	...
	for (int h = 0; h < 건물_도시끝; h++)
	{
         ...
         if (base_h.get_force_id() == force_id and (1 <= func_enemy_city_count(city_h, 1) or 0 < enemy_weight) and 6000 <= pk::get_troops(base_h))
	    ...
	    pk::force@ force = pk::get_force( base.get_force_id());
           ...
           if (0 < c_list.length)
            ...
            if (c_list[i].get_force_id() != base.get_force_id()) continue; 
            ...
            if (c_list[i].mibun == 신분_재야) continue;
                   ...
                   if (!pk::is_unitize(c_list[i]) and !pk::is_absent(c_list[i]) and c_list[i].order == -1 and c_list[i].service != base.get_id() and c_list[i].location != base.get_id() and c_list[i].stat[무장능력_무력] >= 86 and c_list[i].location == c_list[i].service and !c_list[i].action_done and !pk::is_absent(c_list[i]) and (c_list[i].mibun == 신분_군주 or c_list[i].mibun == 신분_도독 or c_list[i].mibun == 신분_태수 or c_list[i].mibun == 신분_일반) and c_list[i].get_force_id() == src_base.get_force_id() and c_list[i].get_force_id() == base.get_force_id() and 0 == func_enemy_city_count(city_b, 1))
                      {
	...
	for (int h = 0; h < 건물_도시끝; h++)
	{
	    ...
	    pk::force@ force = pk::get_force( base.get_force_id());
           ...
           if (0 < c_list.length)
            ...
            if (c_list[i].mibun == 신분_재야) continue;
                      ...
                      if (!pk::is_unitize(c_list[i]) and !pk::is_absent(c_list[i]) and c_list[i].order == -1 and c_list[i].service != base.get_id() and c_list[i].location != base.get_id() and c_list[i].stat[무장능력_지력] >= 86 and c_list[i].location == c_list[i].service and !c_list[i].action_done and !pk::is_absent(c_list[i]) and (c_list[i].mibun == 신분_군주 or c_list[i].mibun == 신분_도독 or c_list[i].mibun == 신분_태수 or c_list[i].mibun == 신분_일반) and c_list[i].get_force_id() == src_base.get_force_id() and c_list[i].get_force_id() == base.get_force_id() and 0 == func_enemy_city_count(city_b, 1))
                      {
                             if (pk::get_city_list(force).count >= 5 and c_list[i].mibun != 신분_군주)
                             candidate86_list.add(c_list[i]);
       ...
       if (actors.count == 0 ) return false;
        ...
        bool needREAR(pk::building@ base)
		{
            ...
            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));
            if (0 < func_enemy_city_count(city_a, 2) or 0 < func_user_city_count(city_a, 1)) return false;			
...
// 최후엔 전방에 잉여 장수 많은 도시에서라도
        ...
        bool needGARRISON(pk::building@ base)
		{
            ...
            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));
            int enemy_weight = countNeighborEnemyBase(base);			
            if ((0 == func_enemy_city_count(city_a, 2) or 0 < func_enemy_city_count(city_a, 1) or 0 < enemy_weight) and 0 == func_user_city_count(city_a, 1)) return false;			
