	...
	class Main
	{
		...
		bool callback(pk::force@ force)
		{
            ...
            int Empty = 0;
                ...
                if (pk::is_alive(dst) and dst.get_force_id() == -1 and pk::get_troops(dst) == 0)
                     {
			...
			if (pk::is_normal_force(force_id) and Empty > 0)
			{
				for (int i = 0; i < 건물_거점끝; i++)
				{
                    ...
                    if ( ( !force.is_player() and base.get_force_id() == force_id and needEmpty(base) ) or ( force.is_player() and pk::is_alive(district) and !pk::is_player_controlled(base) and base.get_force_id() == force_id and needEmpty(base) and  district.attack ) )
		...
		bool needEmpty(pk::building@ base)
		{
			// 공백지가 있는 상태인가
            int Empty = 0;
			...
			int base_hp = base.hp;
	                ...
	                // 병력 2천명 이상 있고, 적 침공에 견딜 수 있는 관문, 항구에서 파병 보내라

			if ( 2000 <= base_troops and 건물_관문시작 <= base.get_id() and base.get_id() < 건물_항구끝 and enemy_troops1 * 2.0f <= base_troops and pk::is_normal_force(base.get_force_id()))
				return true;

	                // 병력 5천명 이상 있고, 적 침공에 견딜 수 있는 도시에서 파병 보내라

			if ( 5000 <= base_troops and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and enemy_troops1 * 2.0f <= base_troops and pk::is_normal_force(base.get_force_id()))
				return true;
			return false;
		}

		/** 전투부대 파견 */
		bool PushEmpty(pk::building@ base)
		{
			// 명령 가능한 무장이 있는지 확인.
       ...
       if (actors.count == 0 ) return false;
			...
			{
                if (main.cmd_archerEMP)    // 노병 우대 출진
                {
            ...
            // 육상 무기 선택
            get_ground_weaponEMP(base, leader, reinforce_troops, ground_weapon_id, unit_troops);
            ...
            // 출진 명령
                ...
                cmd.order = 부대임무_공격;
                cmd.target_pos = pk::get_building(target).get_pos();  // 목표는 타겟 거점

                string target_name = pk::u8decode(pk::get_name(pk::get_building(target)));
	if (대사표시_설정) 
                pk::say(pk::u8encode(pk::format("인근에 공백지가 있구나!\n\x1b[2x{}\x1b[0x(으)로 병력을 보내 속히 점유하자!", target_name)), leader);	
        ...
        // 노병우대 출진여부
        bool cmd_archerEMP = false;    
        
        //----------------------------------------------------------------------------------
        
        // 무기 선택 함수
        void get_ground_weaponEMP (pk::building@ base, pk::person@ leader, int troops_max, int &out weapon_sel, int &out troops_sel)
        {
            int troops_min = 2200;
            int weapon_max = 0;
        ...
        pk::building@ src_emp;
		int getEmptyBase(pk::building@ src)
		{
			int best_dst = -1;
			int best_distance = 0;
      ...
      // 공백지 공격
       ...
       if (src_id != dst_id and pk::is_normal_force(src.get_force_id()) and dst.get_force_id() == -1 and pk::is_alive(src) and dst_id < 건물_거점끝 and base2_troops == 0 and func_already (dst) == 0 
	   and 건물_파양항 != dst_id and 건물_노릉항 != dst_id and 건물_서하항 != dst_id and 건물_하양항 != dst_id and 건물_해현항 != dst_id and 건물_강도항 != dst_id)
            ...
            else
            {
                ...
                {
                    int build_dist_a = pk::get_building_distance(a.get_id(), main.src_emp.get_id(), a.get_force_id());
                    int build_dist_b = pk::get_building_distance(b.get_id(), main.src_emp.get_id(), b.get_force_id());

                    int pos_dist_a = pk::get_distance(a.pos, main.src_emp.pos);
                    int pos_dist_b = pk::get_distance(b.pos, main.src_emp.pos);
      ...
      // 해당 공백지로 미리 출진한 부대 숫자 체크, ﻿masterpiecek님의 AI_도시물자수송.cpp﻿를 참조했습니다.
