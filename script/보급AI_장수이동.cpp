...
// 세력의 장수수가 도시당 7명이 안되면 매월 1일에는 도시로 돌아와
        if (7.0f > float( pk::get_person_list(force, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count / float ( pk::get_city_count(force) )))
        {
...
// 세력의 장수수가 도시당 7명 이상이면 홀수달 1일에는 도시로 돌아와	
        else if (7.0f <= float( pk::get_person_list(force, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count / float ( pk::get_city_count(force) )))
        {
...
// 적 침입 있는 항관에 병력 당 장수 숫자 많으면
       ...
       if (actors.count == 0 ) return false;
        ...
        int getClosestPerson(pk::building@ src)
		{
            ...
            int dst_id = dst.get_id();
                    ...
                    int max_distance = 2;
                    if (!force_s.is_player() and ((0 <= distance and distance < best_distance and distance <= max_distance) or best_dst == -1))
                    {
         ...
         if ( distance <= 2 and i >= 건물_도시시작 and i < 건물_도시끝 and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_force_id() == dst.get_force_id() )			
         {
	...
	// 플레이어 세력과 이민족 세력은 제외. 항구 관문 중 도시 1개만 있을땐 병력 2천 이상, 2개 이상일땐 1만3천 이상, 장수 없는 곳
	...
	// 플레이어 세력, 이민족 세력은 제외. 항구 관문 중 금 3천 이상 있는 곳
           ...
           if (person_list.count + func_already(base) < 1 
		   and 3000 <= pk::get_gold(base) and pk::get_troops(base) >= 6000
		   and 건물_파양항 != base.get_id() and 건물_노릉항 != base.get_id() and 건물_서하항 != base.get_id() and 건물_하양항 != base.get_id() and 건물_해현항 != base.get_id() and 건물_강도항 != base.get_id()  ) 
                return true;
     ...
     // 장수가 3명 이상 있고, 도시의 금이 1만1천 이하 혹은 항구 관문보다 금이 적거나 항구, 관문이 보유한도까지 2만 이하 남은 상태고 장수 보내려는 도시도 한도까지 여유 있으면 장수를 보내라
            int src_count = pk::get_person_list(src, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count;
			pk::force@ force = pk::get_force(dst.get_force_id());
                    if ( distance <= 2)			
                        {
   if (person_list.count > 0 and src_count > 2 and ( pk::get_gold(src) <= 11000 or pk::get_gold(src) <= pk::get_gold(dst) or ( 20000 >= pk::get_max_gold(dst) - pk::get_gold(dst) and 40000 <= pk::get_max_gold(src) - pk::get_gold(src) ) ) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_force_id() == dst.get_force_id() and i < 건물_도시끝)
	...
	// 플레이어 세력, 이민족 세력은 제외. 항구 관문 중 군량 5천 이상에 병력 대비 3.5배 이상 있는 곳으로 장수 보냄
           ...
           if (person_list.count + func_already(base) < 1 and base_troops * 3.5f <= pk::get_food(base) and 10000 <= pk::get_food(base) and 건물_파양항 != base.get_id() and 건물_노릉항 != base.get_id() and 건물_서하항 != base.get_id() and 건물_하양항 != base.get_id() and 건물_해현항 != base.get_id() and 건물_강도항 != base.get_id()  ) 
                return true;
     ...
     // 장수 보낼 도시 조건 : 장수가 3명이상 있고, (병량이 병력 대비 4배 이하 or 병량 5만 이하 or 장수 받을 항관보다 병량이 더 적음 or 항관의 병량 여유 공간이 20만 이하인데 도시 병량 저장 공간이 40만 이상 여유 있음)
     // 장수 받을 항관 조건 : 병량 1만 이상에 병력 대비 병량이 3.5배 이상
            int src_count = pk::get_person_list(src, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count;
			pk::force@ force = pk::get_force(dst.get_force_id());
                    if ( distance <= 2)				
                        {
if (person_list.count > 0 and src_count > 2 and 10000 <= pk::get_food(dst) and pk::get_food(dst) >= pk::get_troops(dst) * 3.5f and (pk::get_troops(src) * 4.0f  >= pk::get_food(src) or pk::get_food(src) <= 50000 or pk::get_food(src) <= pk::get_food(dst) or ( pk::get_max_food(dst) - pk::get_food(dst) <= 200000 and pk::get_max_food(src) - pk::get_food(src) >= 400000 ) ) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_force_id() == dst.get_force_id() and i < 건물_도시끝)
                        {
                            best_src = i;
                            best_distance = distance;
                        }
       ...
       if (actors.count == 0 ) return false;
        ...
        int getExodusPerson(pk::building@ src)
		{
            ...
            // 적의 공격 받는 도시 중에서, 장수가 5명 이상에, 병력이 태수 성격에 따라 3000명 안팎이면 장수를 내보내라, (항구, 관문의 경우에는 장수 2명 이상에, 2000명)
                    ...
                    if (distance <= 2 and pk::is_alive(taishu) and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_force_id() == dst.get_force_id())			
                        {
                        int a = taishu.character;
