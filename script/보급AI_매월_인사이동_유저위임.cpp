...
namespace 매월_인사이동_유저위임utf8
{
	...
	if (pk::is_alive(district) and !pk::is_player_controlled(base) and district.personnel)
        ...
        bool needValor(pk::building@ base)
		{
			...
			// 플레이어 조종 군단이 아님, 이민족 세력은 제외.
			if (!pk::is_player_controlled(district) and pk::is_normal_force(district_id))
			{

            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));
            if (0 == func_enemy_city_count(city_a, 1)) return false;
           ...
           if (0 < b_list.length)
            {
                for (int i = 0; i < int(b_list.length); i++)
                {
                    ...
                    if (b_list[i].stat[무장능력_무력] >= 86 and !b_list[i].action_done   and b_list[i].get_district_id() == base.get_district_id() )
           ...
           pk::list<pk::person@> district86_list;
		   district86_list.clear();		   

            auto f_list = pk::list_to_array(pk::get_person_list(district, pk::mibun_flags(신분_군주,신분_도독,신분_태수,신분_일반) ) );
           ...
           if (0 < f_list.length)
            {
                for (int i = 0; i < int(f_list.length); i++)
                {
					...
					if (f_list[i].stat[무장능력_무력] >= 86 and !f_list[i].action_done and !pk::is_absent(f_list[i]) and f_list[i].get_district_id() == base.get_district_id() )

                    district86_list.add(f_list[i]);

	        }

	    }
                             
       if (district86_list.count == 0 ) return false;   // 86 이상 무장 부족
	...
	for (int h = 0; h < 건물_도시끝; h++)
	{
	     int district_id = district.get_id();
	     pk::building@ base_h  = pk::get_building(h);
         pk::city@ city_h = pk::get_city(pk::get_city_id(base_h.pos));
         int enemy_weight = countNeighborEnemyBase(base_h);

         if (base_h.get_district_id() == district_id and (1 <= func_enemy_city_count(city_h, 1) or 0 < enemy_weight) and 6000 <= pk::get_troops(base_h))
        ...
        if (1 <= func_enemy_city_count(city_a, 1) and 6000 <= pk::get_troops(base) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and base86_list.count < float (district86_list.count/ float (hotzone_count)) ) 
        {
	    ...
	    pk::district@ district = pk::get_district( base.get_district_id());
            ...
            int person_count = pk::get_person_list(district, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count;
            ...
            if (above86count(district, 무장능력_무력)  == 0 ) return false;   // 세력 내 86 이상 무장 부재
		   ...
		   candidate86_list.clear();		   

            auto c_list = pk::list_to_array(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반) ) );
           ...
           if (0 < c_list.length)
            ...
            if (c_list[i].get_district_id() != src_base.get_district_id()) continue; 
            if (c_list[i].get_district_id() != base.get_district_id()) continue; 
            ...
            if (c_list[i].mibun == 신분_재야) continue;
                   ...
                   if (!pk::is_unitize(c_list[i]) and !pk::is_absent(c_list[i]) and c_list[i].order == -1 and c_list[i].service != base.get_id() and c_list[i].location != base.get_id() and c_list[i].stat[무장능력_무력] >= 86 and c_list[i].location == c_list[i].service and !c_list[i].action_done and !pk::is_absent(c_list[i]) and (c_list[i].mibun == 신분_도독 or c_list[i].mibun == 신분_태수 or c_list[i].mibun == 신분_일반) and c_list[i].get_district_id() == src_base.get_district_id() and c_list[i].get_district_id() == base.get_district_id() and 0 == func_enemy_city_count(city_b, 1))
                      {
           ...
           if (0 < b_list.length)
            {
                for (int i = 0; i < int(b_list.length); i++)
                {
                    pk::person@ person = b_list[i];
                             ...
                             if (b_list[i].stat[무장능력_지력] >= 86 and !b_list[i].action_done   and b_list[i].get_district_id() == base.get_district_id() )
           ...
           if (0 < f_list.length)
            {
                for (int i = 0; i < int(f_list.length); i++)
                {
                    pk::person@ person = f_list[i];
                             ...
                             if (f_list[i].stat[무장능력_지력] >= 86 and !f_list[i].action_done and !pk::is_absent(f_list[i]) and f_list[i].get_district_id() == base.get_district_id() )
	...
	for (int h = 0; h < 건물_도시끝; h++)
	{
	    ...
	    pk::district@ district = pk::get_district( base.get_district_id());
            ...
            if (above86count(district, 무장능력_지력)  == 0 ) return false;   // 세력 내 86 이상 무장 부재
           ...
           if (0 < c_list.length)
            ...
            if (c_list[i].mibun == 신분_재야) continue;
                      ...
                      if (!pk::is_unitize(c_list[i]) and !pk::is_absent(c_list[i]) and c_list[i].order == -1 and c_list[i].service != base.get_id() and c_list[i].location != base.get_id() and c_list[i].stat[무장능력_지력] >= 86 and c_list[i].location == c_list[i].service and !c_list[i].action_done and !pk::is_absent(c_list[i]) and (c_list[i].mibun == 신분_도독 or c_list[i].mibun == 신분_태수 or c_list[i].mibun == 신분_일반) and c_list[i].get_district_id() == src_base.get_district_id() and c_list[i].get_district_id() == base.get_district_id() and 0 == func_enemy_city_count(city_b, 1))
                      {
       ...
       if (actors.count == 0 ) return false;
        ...
        bool needREAR(pk::building@ base)
		{
            ...
            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));	
            if (0 < func_enemy_city_count(city_a, 2)) return false;			
			...
			// 플레이어 조종 거점과 이민족 세력은 제외하고 평균적으로 무장수가 적다고 판단되면 대상
			if (!pk::is_player_controlled(district) and !pk::is_player_controlled(base) and pk::is_normal_force(district_id))
			{
 ...
 if ( (count + func_already(base) <= 0.5f * float(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_list(district).count ))) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and 0 == func_enemy_city_count(city_a, 2) ) 
                return true;
			
 if ( (count + func_already(base) <=  1.0f * float(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_list(district).count) ) ) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and 0 == func_enemy_city_count(city_a, 2) and !check_city_dev_complete(city_a)) 
                return true;			
            }        
            return false;
        }
        ...
        bool PushREAR(pk::building@ dst_base)
        {
            ...
            // 장수 보내는 곳과 받는 곳 군단이 같아야 하고, 받는 곳 장수 숫자가 보내는 곳 장수 숫자보다 그렇게 많이 적지 않으면 보내지 말것
		if (dst_base.get_district_id() != src_base.get_district_id()) return false;
		...
		if (pk::get_person_list(dst_base, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count + 2 >= pk::get_person_list(src_base, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count) return false;
            ...
            if (count <  float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district)) ) * 0.2f ) return false;   // 무장 부족
...
return (a.stat[무장능력_무력] + a.stat[무장능력_통솔] + a.stat[무장능력_지력] + a.stat[무장능력_정치] + a.stat[무장능력_매력]) < (b.stat[무장능력_무력] + b.stat[무장능력_통솔] + b.stat[무장능력_지력] + b.stat[무장능력_정치] + b.stat[무장능력_매력]);
            ...
            // 소환 대상 무장 (소속과 소재거점이 다르면 제외, 같고 보내려는 장수와 보내는 도시의 군단이 같으면 보내라)
            pk::list<pk::person@> actors;
            actors.clear();				
            for (int i = 0; i < max; i++)
            {
       ...
       if (actors.count == 0 ) return false;
        ...
        int getEXTRAPerson(pk::building@ dst)
		{
			...
			for (int i = 0; i < 건물_도시끝; i++)
			{
				...
				if (dst.get_id() != i and dst.get_district_id() == src.get_district_id()  )
                {
	   ...
	   pk::district@ district_s = pk::get_district(src.get_district_id());
                    ...
                    int max_distance = 12;
                    if (!pk::is_player_controlled(district_s) and (0 <= distance and distance <= max_distance))
                    {
...
if ( person_idle_list.count > 0 and (count > count_dst + 2 or count > 0.7f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and check_city_dev_complete(city_d) )
                        {
                        best_src = src_id;
...
if ( person_idle_list.count > 0 and (count > 2.5f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and !check_city_dev_complete(city_d) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( person_idle_list.count > 0 and (count > 11 or count > 1.3f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and 0 == enemy_weight and check_city_dev_complete(city_d))
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( person_idle_list.count > 0 and (count > 2.5f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and 0 == enemy_weight and !check_city_dev_complete(city_d))
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
// 최후엔 전방에 잉여 장수 많은 도시에서라도
...
else if ( person_idle_list.count > 10 and (count > 22 or count > 3.1f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id()  and (1 <= func_enemy_city_count(city_d, 1) or 0 < enemy_weight) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }	
            ...
            auto person_list_b = pk::get_idle_person_list(b);
                    ...
                    if (person_list_a.count - build_dist_a == person_list_b.count - build_dist_b and build_dist_a != build_dist_b) 
        ...
        bool needGARRISON(pk::building@ base)
		{
            ...
            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));	
            int enemy_weight = countNeighborEnemyBase(base);			
            if (0 == func_enemy_city_count(city_a, 2) or 0 < func_enemy_city_count(city_a, 1) or 0 < enemy_weight) return false;			
 ...
 if ( (count + func_already(base) <= 0.8f * float(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float ( pk::get_city_list(district).count ))) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and (1 <= func_enemy_city_count(city_a, 2) and 0 == func_enemy_city_count(city_a, 1) )) 
                return true;
	 
 if ( (count + func_already(base) <=  1.0f * float(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float( pk::get_city_list(district).count)) ) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and (1 <= func_enemy_city_count(city_a, 2) and 0 == func_enemy_city_count(city_a, 1)) and !check_city_dev_complete(city_a) )
                return true;	 
...
if ( 0 == func_enemy_city_count(city_s, 2) and dst_base.get_district_id() == src_base.get_district_id())
...
if ( ( 1 <= func_enemy_city_count(city_s, 1) or ( 1 <= func_enemy_city_count(city_s, 2) and 0 == func_enemy_city_count(city_s, 1) ) ) and dst_base.get_district_id() == src_base.get_district_id() )
...
if ( person_idle_list.count > 0 and (count >= count_dst or count > 0.7f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and check_city_dev_complete(city_d) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( person_idle_list.count > 0 and (count > count_dst + 2 or count > 1.3f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and 0 == enemy_weight and check_city_dev_complete(city_d) and count > count_dst + 2)
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
// 최후엔 전방에 잉여 장수 많은 도시에서라도
...
else if ( person_idle_list.count > 10 and (count > 22 or count > 3.1f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id()  and (1 <= func_enemy_city_count(city_d, 1) or 0 < enemy_weight) and count > count_dst + 2)
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
       ...
       //  전방 도시에 무장 수가 부족하면 무장 지원
        ...
        bool needFRONT(pk::building@ base)
		{
            ...
            pk::city@ city_a = pk::get_city(pk::get_city_id(base.pos));	
            int enemy_weight = countNeighborEnemyBase(base);			
            if (0 == func_enemy_city_count(city_a, 1) and 0 == enemy_weight) return false;			
 ...
 if ( (count + func_already(base) <= 2.5f * float(pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_list(district).count ))) and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝 and (1 <= func_enemy_city_count(city_a, 1) or 0 < enemy_weight)) 
                return true;
            }        
            return false;
        }
...
if ( ( 0 == func_enemy_city_count(city_s, 2) or ( 1 <= func_enemy_city_count(city_s, 2) and 0 == func_enemy_city_count(city_s, 1) ) ) and dst_base.get_district_id() == src_base.get_district_id())
...
if ( 1 <= func_enemy_city_count(city_s, 1) and dst_base.get_district_id() == src_base.get_district_id() )
...
if ( person_idle_list.count > 0 and (count >= count_dst or count >  0.7f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and ( check_city_dev_complete(city_d) or 0 < enemy_units3 ) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( person_idle_list.count > 0 and (count >= count_dst or count >  1.3f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and 0 == enemy_weight and ( check_city_dev_complete(city_d) or 0 < enemy_units3 ) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
// 최후엔 전방에 잉여 장수 많은 도시에서라도 (세력의 도시 모두 전방인 경우와 아닌 경우 나눠서)
...
else if (pk::get_city_list(district).count == func_FRONT_city_count(district) and person_idle_list.count > count_dst and (count > count_dst + 2 or count >  1.2f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id()  and (1 <= func_enemy_city_count(city_d, 1) or 0 < enemy_weight) and count > count_dst + 2)
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }

else if (pk::get_city_list(district).count > func_FRONT_city_count(district) and person_idle_list.count > 10 and (count > count_dst + 2 or count >  1.5f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id()  and (1 <= func_enemy_city_count(city_d, 1) or 0 < enemy_weight) and count > count_dst + 2)
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
        ...
        bool needUnitleader(pk::building@ base)
		{
			...
			// 플레이어 조종 거점, 이민족 세력은 제외하고 병력 5천 당 장수 수가 적고, 전방이면 우선순위
			if (!pk::is_player_controlled(district) and !pk::is_player_controlled(base) and pk::is_normal_force(district_id))
			{
...
if (count + func_already(base) <= float ( pk::get_troops(base) / 5000.f ) and (1 <= func_enemy_city_count(city_a, 1) or 0 < enemy_weight) and 건물_파양항 != base.get_id() and 건물_노릉항 != base.get_id() and 건물_서하항 != base.get_id() and 건물_하양항 != base.get_id() and 건물_해현항 != base.get_id() and 건물_강도항 != base.get_id() and 건물_도시시작 <= base.get_id() and base.get_id() < 건물_도시끝) 
		...
		if (dst_base.get_district_id() != src_base.get_district_id()) return false;
            ...
            auto person_list = pk::get_idle_person_list(src_base);
            if (person_list.count <  float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float(pk::get_city_count(district) ))  * 0.2f ) return false;   // 무장 부족
...
if ( (person_list.count >  0.7f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and ( check_city_dev_complete(city_d) or 0 < enemy_units3 ) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }

if ( (person_list.count >  2.5f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 0 == func_enemy_city_count(city_d, 2) and  0 == enemy_weight and !check_city_dev_complete(city_d) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( (person_list.count >  1.3f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and  0 == enemy_weight and ( check_city_dev_complete(city_d) or 0 < enemy_units3 ) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
else if ( (person_list.count >  2.5f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 1 <= func_enemy_city_count(city_d, 2) and  0 == func_enemy_city_count(city_d, 1) and  0 == enemy_weight and !check_city_dev_complete(city_d) )
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
...
// 최후엔 전방에 잉여 장수 많은 도시에서라도
...
else if ( (person_list.count >  3.1f * float( pk::get_person_list(district, pk::mibun_flags(신분_도독,신분_태수,신분_일반)).count / float (pk::get_city_count(district) ))) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id()  and (1 <= func_enemy_city_count(city_d, 1) or 0 < enemy_weight) and pk::get_person_list(dst, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count + 2 < pk::get_person_list(src, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반)).count)
                        {
                        best_src = src_id;
                        src_list.add(src);  // 수송가능 거점리스트 추가
                        }
	...
	int above86count(pk::district@ district, int attributes)
	{
  ...
  if (pk::is_alive(district))
        {
            auto list = pk::list_to_array(pk::get_person_list(district, pk::mibun_flags(신분_군주, 신분_도독, 신분_태수, 신분_일반) ) );
            if (0 < list.length)
            {
...
if (src_id != dst_id and pk::is_neighbor_base(src_id, dst_id) and pk::is_enemy(src, dst) and (건물_파양항 != dst_id and 건물_노릉항 != dst_id and 건물_서하항 != dst_id and 건물_하양항 != dst_id and 건물_해현항 != dst_id and 건물_강도항 != dst_id ) )
		...
		int func_FRONT_city_count(pk::district@ district)
		{
			...
			for (int i = 0; i < 도시_끝; i++)
			{
				...
				if (pk::is_alive(FRONTcity) and pk::is_normal_force(district_id) and FRONTcity.get_district_id() == district_id and 1 <= func_enemy_city_count(FRONTcity, 1)) 
