namespace evolutionkit
{
	class Main
	{
		...
		void func168(pk::building@ base, pk::unit@ attacker)
		{
			...
			{
			n = pk::min(pk::max(charisma / 4, 5), 100);				
		...
		int func253(pk::building@ src, int src_rank, pk::building@ dst, int dst_troops)
		{
			...
			if (src.get_id() < 건물_도시끝 and dst_troops > dst_max_troops)
				return dst_troops + dst_troops / 4;
			else if (src.get_id() < 건물_도시끝 and dst_troops * 2 > dst_max_troops)
				return dst_max_troops + dst_max_troops / 4;

			else if (src.get_id() < 건물_도시끝 )
				return pk::max(dst_troops, dst_max_troops / 4) * 3 ;

			else if (src.get_id() >= 건물_도시끝 and dst_troops > dst_max_troops)
				return pk::min (  dst_troops + dst_troops / 4 , pk::get_troops(src) /3);

			else if (src.get_id() >= 건물_도시끝 and dst_troops * 2 > dst_max_troops)
				return pk::min ( dst_max_troops + dst_max_troops / 4 , pk::get_troops(src) /3);

			else if (src.get_id() >= 건물_도시끝)
				return pk::min ( pk::max(dst_troops, dst_max_troops / 4) * 3 , pk::get_troops(src)/3);
			else
				return pk::min (pk::max(dst_troops, dst_max_troops / 4) * 3  , pk::get_troops(src)/t);
		}
		...
		void func255(pk::ai_context@ context, pk::building@ building)
		{
			...
			if (base.enemies_around3)
			{
             ...
             if (building_id >= 건물_도시시작 and building_id < 건물_도시끝 and 10000 < pk::get_troops(building)  )
			 {																			 		  																												  
				context.push_cmd(거점AI_방어);
			 }
