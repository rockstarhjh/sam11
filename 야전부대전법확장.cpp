...
namespace 야전부대전법확장
{
	...
	class Main
	{
        ...
        {
            pk::get_tactics(전법_공성화시).name = pk::encode("공성화시");
        }
        ...
        void backup_tactics_info()
        {
            for (int i = 0; i < 전법_끝; i++)
            {
                ...
                주가전법[i] = pk::get_equipment(병기_주가).tactics[i];
                누선전법[i] = pk::get_equipment(병기_누선).tactics[i];
                투함전법[i] = pk::get_equipment(병기_투함).tactics[i];
            }
        }
        ...
        void reset_tactics_info()
        {
            for (int i = 0; i < 전법_끝; i++)
            {
                ...
                pk::get_equipment(병기_주가).tactics[i] = 주가전법[i];
                pk::get_equipment(병기_누선).tactics[i] = 누선전법[i];
                pk::get_equipment(병기_투함).tactics[i] = 투함전법[i];
            }
        }
