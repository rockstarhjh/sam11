       ...
       if (actors.count == 0 ) return false;
        ...
        int getClosestPerson(pk::building@ dst)
		{
			...
			pk::district@ district = pk::get_district(dst.get_district_id());
                    ...
                    if ( distance <= 2 and i >= 건물_관문시작 and i < 건물_거점끝 and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_district_id() == dst.get_district_id())				
                        {
...
// 도시에 장수 15명 이하에도 감				
       ...
       if (actors.count == 0 ) return false;
        ...
        int getNearbyPerson(pk::building@ dst)
		{
			...
			pk::district@ district = pk::get_district(dst.get_district_id());
                    if ( distance <= 2)			
                        {
       if ( person_list.count >= 6 and person_list.count >= pk::get_troops(src) / 4000.f and i >= 건물_관문시작 and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_district_id() == dst.get_district_id())
                        {
                            best_src = i;
                            best_distance = distance;
                        }
	...
	// 플레이어 조종 거점, 이민족 세력은 제외. 항구 관문 중 병력 2천 이상, 장수 없는 곳
   ...
   if (person_list.count > 0 and src_count > 2 and ( pk::get_gold(src) <= 11000 or pk::get_gold(src) <=  pk::get_gold(dst)  or ( 20000 >= pk::get_max_gold(dst) - pk::get_gold(dst) and 40000 <= pk::get_max_gold(src) - pk::get_gold(src) )) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and i < 건물_도시끝)
	...
	// 플레이어 조종 거점, 이민족 세력은 제외. 항구 관문 중 군량 5천 이상에 병력 대비 2배 이상 있는 곳으로 장수 보냄
     ...
     //  장수가 3명이상 있고, 군량이 3만 이하거나 병력 대비 0.8이하거나 항구, 관문 보다 군량이 적거나, 항구 관문에 군량이 병력대비 3.5배 이상이거나 도시의 군량이 3만 이하 혹은 병력대비 2.5 이하 혹은 항관의 병량이 한도치까지 20만 이하로 남았고 도시는 아직 여유있으면 항구, 관문에 장수를 보내라
			...
			pk::district@ district = pk::get_district(dst.get_district_id());
                    if ( distance <= 2)				
                        {
if (person_list.count > 0 and src_count > 2 and (pk::get_food(src) <= 30000 or pk::get_food(src) <= base1_troops * 0.8f or pk::get_food(src) <= pk::get_food(dst) or ( 10000 <= pk::get_food(dst) and pk::get_food(dst) >= pk::get_troops(dst) * 3.5f  and  (  pk::get_troops(src) * 2.5f  >= pk::get_food(src)  ) )  or  ( pk::get_max_food(dst) - pk::get_food(dst) <= 200000 and pk::get_max_food(src) - pk::get_food(src) >= 400000 )  ) and pk::is_alive(src) and pk::is_alive(dst) and src_id != dst_id and src.get_district_id() == dst.get_district_id() and i < 건물_도시끝)
                        {
                            best_src = i;
                            best_distance = distance;
                        }
	...
	// 플레이어 조종 거점, 이민족 세력은 제외. 항구 관문 중 물자,병력 있는 위급거점으로 보냄
           ...
           if (person_list.count + func_already(base) < 3 and pk::enemies_around(base) and ( ( base.hp < 800 and ( 5000 <= base_troops or 4000 <= pk::get_gold(base) or 8000 <= pk::get_food(base) ) ) or ( base_troops <= 2500 and ( 450 <  base.hp or 4000 <= pk::get_gold(base) or 8000 <= pk::get_food(base) ) ) )  and 건물_파양항 != base.get_id() and 건물_노릉항 != base.get_id() and 건물_서하항 != base.get_id() and 건물_하양항 != base.get_id() and 건물_해현항 != base.get_id() and 건물_강도항 != base.get_id() ) 
                return true;
        ...
        int getExodusPerson(pk::building@ dst)
		{
            ...
            // 적의 공격 받는 도시 중에서, 장수가 5명 이상에, 병력이 태수 성격에 따라 3000명 안팎이면 장수를 내보내라, (항구, 관문의 경우에는 장수 2명 이상에, 2000명)
                   ...
                   pk::person@ taishu = pk::get_person(pk::get_taishu_id(src));
                    ...
                    if ( distance <= 2 and pk::is_alive(taishu) and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 건물_도시시작 <= i and i < 건물_도시끝)				
                        {
                    ...
                    if ( distance <= 2 and pk::is_alive(taishu) and pk::is_alive(src) and pk::is_alive(dst)  and src_id != dst_id and src.get_district_id() == dst.get_district_id() and 건물_관문시작 <= i and i < 건물_항구끝)				
                        {
                        int a = taishu.character;
